...
class Content(object):
    ...
    @staticmethod
    def save_to_file(all_content, str_split=':', path='./proxyip.txt'):
        """
        把数据存到文件中
...
class Proxy(object):
    ...
    def test_proxy(self):
        ...
        for proxy in all_ips:
            ...
            ip_info['proxy_host'] = proxy.split(':')[0]
            ip_info['proxy_port'] = proxy.split(':')[1]
            try:
                ...
                datainfo = ip_info
                datainfo['_id'] = datainfo['proxy_host'] + ':' + datainfo['proxy_port']
                datainfo['status'] = True
                # 将可以使用的proxy 存入数据库
                try:
                    dbproxy.insert_one(datainfo)
                except:
                    pass
        ...
        # 返回测试过，可用的代理
        print('经测试：{}个可用，可用率：{}%'.format(len(success_proxy),len(success_proxy)/len(all_ips)))
    ...
    def fetch_kxdaili(self):
        ...
        # ips_list = [] get_soup
        # html_body = self.response.body
        html_body = get_soup(self.response)
        proxyes = []
        table_tag = html_body.find("table", attrs={"class": "segment"})
        try:
        ...
        self.proxies = proxyes
        # return self.test_proxy()
        proxies =  yield self.test_proxy()
        # raise gen.Return(self.test_proxy())
        return proxies
    ...
    def fetch_xici(self):
        ...
        try:
            ...
            for fi_ip_info in ip_list_table.find_all('tr'):
                ip_detail = fi_ip_info.find_all('td')
                if ip_detail:
                    proxyes.append(str(list(ip_detail)[1].string) + ':' + str(list(ip_detail)[2].string))
