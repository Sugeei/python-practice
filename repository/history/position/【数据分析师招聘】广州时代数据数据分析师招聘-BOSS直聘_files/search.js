var Search = {
	init: function() {
		...
		Search.searchBox.find('.dorpdown-province li').on('mouseover', function() {
			...
			var $lis = elsCon.eq(idx).find("li");
			if($lis.length>0 && elsCon.eq(idx).find("li.cur").length <1){
				$lis.eq(0).addClass("cur")
			};
		});
		Search.searchBox.find('.dorpdown-province li').on('click', function() {
			var     idx = $(this).index();
		    var     elsCon = Search.searchBox.find('.dorpdown-city ul');
			var     $ul = elsCon.eq(idx);
		...
		Search.searchBox.find('.ipt-search').keydown(function(e) {
			...
			switch(e.which) {
				case 38: // Up arrow
					...
					Search.scrollVisiable(els.eq(currentSelection),"up");					
				break;
				case 40: // Down arrow
					e.preventDefault();
					els.removeClass('selected');
					...
					els.eq(currentSelection).addClass('selected');
					Search.addInput(els.eq(currentSelection), true);
					Search.scrollVisiable(els.eq(currentSelection),"down");
				break;
				case 13: // Enter
					currentSelection = -1;
					break;
	...
	/*搜索建议按方向键时滚动到可视区域*/
	scrollVisiable: function(el,arron) {
		var el = el,
			suggestCon = Search.searchBox.find('.suggest-result ul');

		if(!el){ return false;}

		var     $len = suggestCon.find("li").length;
		var   $index = $(el).index();
		var     $tar = ($index  > 4) ? $index  - 4 : 0;
		var   enDown = arron == "down" && (($tar < ($len-4) ) || ($index === 0));
		var     enUp = arron == "up" && ($index < $len - 5 || $index ==$len-1); 
		if(arron == "up"){
			$tar = $index;
		}
		if(enUp || enDown){
			suggestCon.scrollTop($tar*$(el).height());
		}		
	}
};