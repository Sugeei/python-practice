...
class FeedSZSE(FeedBase):
    def __str__(self):
        return u"深圳证券交易所"

    def __init__(self):
        self.shortname = 'sz_se_listed_info'
        self.url = 'http://www.szse.cn/certificate/individual/index.html?code=%s'

    def get_src_url(self, key):
        # logger.info("valid usl=%s" % (self.url % key))
        return self.url % key
