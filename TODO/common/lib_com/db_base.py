...
class DB_Base:
    ...
    def update_2_table(self, tablename, df_data, uniq_keys, conn):
        if df_data is not None and len(df_data) > 0:
            ...
            conn.commit()
            cur.close()
        logger.info("update database real_count = %s" % (len(df_data)))
    ...
    def merge_2_table(self, df_data, tablename, union_keys, where_condition, conn):
        if df_data is not None and len(df_data) > 0:
            ...
            logger.info("merge_2_table df_data_insert %s" % len(df_data_insert))
            logger.info("merge_2_table df_data_update %s" % len(df_data_update))
